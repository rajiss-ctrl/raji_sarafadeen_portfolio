...
const Resume = () => {
  ...
  return (
      <div onClick={handleNavClick}  >
            {/* SECTION TO PRINT */}
            <div onClick={handleNavClick} id="print-section" className="">
              <h1 className="text-4xl lg:text-5xl font-bold">RAJI SARAFADEEN</h1>
              <div className="flex flex-col md:flex-row gap-2">
                <Link onClick={handleNavClick} href="mailto:omosanjos77@gmail.com" className="text-blue-500 flex gap-1 items-center">
                  <FaEnvelope className="text-[#6b9acc]" /> <span>omosanjos77@gmail.com</span>
                </Link>
                <Link onClick={handleNavClick} href="tel:+2347038699659" className="text-blue-500 flex gap-1 items-center">
                  <BsPhoneVibrate className="text-[#6b9acc]" /> <span>+234 7038699659</span>
                </Link>
              </div>
              <Link
              ...
              <p onClick={handleNavClick} className="text-[#6b9acc] mt-4 font-light">
              ...
              {/* SKILLS */}
              <div onClick={handleNavClick} className="relative overflow-hidden w-full my-12 lg:my-20">
                ...
                <div className="relative overflow-hidden flex flex-col lg:flex-row justify-between items-center pr-8 py-8">
                  ...
                  <div>
                    <h2 className="text-[#6b9acc] font-light">I constantly try to improve</h2>
                    <h3 className="text-3xl">My tech stack</h3>
                  </div>
                  <div className="grid grid-cols-3 lg:grid-cols-4 gap-4">
                    {techStack.map((skill) => (
                      <div key={skill.id} className="bg-[#191d2b] p-2 rounded-[6px] text-[#6b9acc] font-light">
                        {skill.name}
                      </div>
                    ))}
                  </div>
              ...
              {/* WORK EXPERIENCE */}
              <div onClick={handleNavClick} className="relative overflow-hidden w-full my-12 lg:my-20 font-light">
                ...
                <div className="pt-10">
                  <h1 className="text-xl lg:text-2xl font-bold mb-8 flex items-center gap-3">
                    <BsBriefcase size={26} className="text-[#6b9acc] font-light" /> <span>Work Experience</span>
                  </h1>
                    ...
                    <div className="space-y-8">
                      {workExperience.map((exp, index) => (
                        <div key={index} className="relative pl-8 flex flex-col lg:flex-row">
                          ...
                          <h2 className="text-xl text-[#6b9acc] font-light mb-1 lg:w-1/4">{exp.period}</h2>
              ...
              {/* EDUCATION */}
              <div onClick={handleNavClick} className="-mt-10 pb-10 font-light">
                <h1 className="text-xl lg:text-2xl font-bold mb-8 flex items-center gap-3">
                  <BiBook size={26} className="text-[#6b9acc] font-light" /> <span>Educational Qualifications</span>
                </h1>
                <div className="relative">
                  <div className="absolute left-1.5 top-0 h-full w-[0.20rem] bg-[#2e344e]"></div>
                  <div className="space-y-8">
                    {education.map((edu, index) => (
                      <div key={index} className="relative pl-8 flex flex-col lg:flex-row">
                        <div className="absolute z-[60] left-2 top-1.5 h-4 w-4 rounded-full bg-[#10121b] border-3 border-[#2e344e] transform -translate-x-1/2"></div>
                        <h2 className="text-xl text-[#6b9acc] font-light mb-1 lg:w-1/4">{edu.period}</h2>
                        <div className="lg:w-3/4">
                          <h3 className="text-xl font-medium text-blue-600 mb-1 relative before:bg-[#2e344e] before:absolute before:content-[''] before:h-[0.12rem] before:w-[40px] before:-left-14 before:top-1/2">
                            {edu.title}
                          </h3>
                          <h4 className="text-md font-medium text-white mb-2">{edu.school}</h4>
                          <p className="text-[#6b9acc]">{edu.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
